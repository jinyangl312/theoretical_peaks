...
def calc_b_ions(peptide, modinfo, xlmassinfo=None):
    ...
    if xlmassinfo == None:
        ...
        for i in range(len(peptide)-1):
            mass_nterm += aamass.aa_mass_dict[peptide[i]] + modmass_list[i+1] + xlmassinfo[i+1]
            b_ions.append(mass_nterm)
        pepmass = b_ions[-1] + aamass.aa_mass_dict[peptide[-1]] + \
            modmass_list[len(peptide)] + modmass_list[len(peptide)+1] + \
            xlmassinfo[len(peptide)] + xlmassinfo[len(peptide)+1] + \
            aamass.mass_H2O
